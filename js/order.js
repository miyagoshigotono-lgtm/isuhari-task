...
const Order = (() => {
  ...
  function save(orders) {
    ...
    if (typeof DataStore !== 'undefined') {
      DataStore.scheduleSync('save-orders');
    }
  }
  ...
  function submit() {
    ...
    if (editingOrderId) {
      ...
      const noteHistory = (prev && prev.noteHistory) ? [...prev.noteHistory] : [];
      ...
      const newOrder = {
        ...
        noteHistory: note ? [{ at: formatStamp(), before: '', after: note }] : [],
      };
  ...
  function exportAllData() {
    return {
      ...
      orders: getAll(),
      items: Master.getItems(),
      clients: Master.getClients(),
      schedule: Calendar.scheduleData(),
    };
  }

  function importAllData(payload) {
    if (!payload || typeof payload !== 'object') return false;
    localStorage.setItem(ORDER_KEY, JSON.stringify(payload.orders || []));
    if (Array.isArray(payload.items)) {
      localStorage.setItem('isuhari_items', JSON.stringify(payload.items));
    }
    if (Array.isArray(payload.clients)) {
      localStorage.setItem('isuhari_clients', JSON.stringify(payload.clients));
    }
    if (payload.schedule && typeof payload.schedule === 'object') {
      localStorage.setItem('isuhari_schedule', JSON.stringify(payload.schedule));
    }
    if (typeof DataStore !== 'undefined') {
      DataStore.scheduleSync('import-data');
    }
    return true;
  }

  return { init, getAll, save, checkOverflow, removeById, updateById, startEdit, cancelEdit, exportAllData, importAllData };
})();
